...
int main(int argc, char *argv[]){
    Vector3d ba0(0,0,0);
    Vector3d bg0(0,0,0);
    IMU_Preintegration IMUP(ba0,bg0,0.019,0.015);
    vector< vector<double> > imudata;
    imudata = read_data(argv[1]);

    // typedef g2o::BlockSolver<g2o::BlockSolverTraits<300,300>> Block;
    ...
    for(int j = 0; j < 20; j++){
        ...
        double error = (std::rand()/RAND_MAX-0.5)/50 + 1;

        Vector15d state_j;

        state_j<<Log(qj.toRotationMatrix()), tj*error, vj*error, Vector6d::Zero();
        ...
        Vector6d t21_meas;
        t21_meas<<Log(qj.toRotationMatrix()*qi.toRotationMatrix().transpose()),tj-qj.toRotationMatrix()*qi.toRotationMatrix().transpose()*ti;
        ...
        IMUP.reset(ba0,bg0,0.019,0.015);
        qi = qj;
        ti = tj;
    ...
    for(int i = 0; i < 21; i++){
        ...
        //std::cout<<"v:"<<v_->estimate()<<std::endl;
        ...
        if(i==20){
            ...
            for(int j = 0; j < 20; j++){
                ...
                double dt = imudata[i*20+j+1][0]-imudata[i*20+j][0];
                gyro << imudata[i*20+j][8],imudata[i*20+j][9],imudata[i*20+j][10];
                acc << imudata[i*20+j][11],imudata[i*20+j][12],imudata[i*20+j][13];
